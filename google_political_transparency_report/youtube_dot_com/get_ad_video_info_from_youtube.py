...
def scrape_new_ads():
    ...
    with ydl:
        ...
        log1 = "scraped {} videos (source: transparency site) from YouTube in {}; {}s per video".format(len(scraped_youtube_video_ads), str(duration).split(".")[0], len(scraped_youtube_video_ads) / duration.total_seconds())
        ...
        log1 = "scraped {} videos (source: observations) from YouTube in {}; {}s per video".format(len(observed_youtube_video_ads), str(duration).split(".")[0], len(observed_youtube_video_ads) / duration.total_seconds())
        log2 = "success: {}; error: {} (private: {}, unavailable: {})".format(success, error_count, private_count, unavailable_count)
        log.info(log1)
        log.info(log2)
